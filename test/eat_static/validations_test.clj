...
(deftest blended-overrides
  ...
  (is (blend aint [:i -a] a1b8))
  (is (blend aint2 [:i -a] aRb8))
  (is (= {:a 1 :b 8} (eval '(d aint))))
  (is (= {:a 99 :b 8} (eval '(mc aint :a 99))))
  (is (= {:a 123 :b 8} (eval '(mc aint2 :a 123))))
  (is (blend aintd [[a -22]] aint))
  (is (= {:a -22 :b 8 :y "hi there"} (eval '(mc aintd :y "hi there"))))
  (is (not (false? (aintd? {}))))
  (is (not (false? (aintd? {:a -22}))))
  (is (not (false? (aintd? {:a 1000}))))
  (is (false? (aintd? {:a :hi})))
  (is (false? (aintd? {:a 1.1})))
  (is (false? (c aintd? :a :hi)))
  ...
  (is (= {:a :hi :b 8} (mc aRb8 :a :hi)))
  (is (blend b8 [[a 9]] a1b8))
...
(deftest all-same-defaults
  ...
  (is (= {:y 6} (d y6a) (make y6 {})
         (d noy) (d noy2) (d noy3) (d noy4) (d noy5) (d noy6)))
  (is (thrown? AssertionError (eval '(mc noy6 :x 3.3))))
  (is (= {:x 55 :y 6} (eval '(mc noy6 :x 55)) (make noy6 {:x 55})))
  (is (thrown? AssertionError (eval '(mc noy6 :w 1))))
  (is (= {:w 55.1 :y 6} (eval '(mc noy6 :w 55.1)) (make noy6 {:w 55.1})))
  (is (blend conflict [:i -w] noy6))
  (is (thrown? AssertionError (make noy6 {:w 2})))
  (is (thrown? AssertionError (make noy6 {:x 2.1})))
  (is (blend req [w] noy3))
  (is (false? (eval '(c req? :w 12 :y 6))))
  (is (= {:w 1.1 :y 78} (mc noy6 :w 1.1 :y 78))))
